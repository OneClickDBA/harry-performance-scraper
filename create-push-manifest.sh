#!/bin/bash
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "Usage: $0 VERSION REGISTRY" >&2
  exit 1
fi

VERSION="$1"  # Scraper version
REGISTRY="$2" # Container registry/repository

# Tag the images
docker tag "scraper-amd64:latest" "${REGISTRY}:${VERSION}-amd64"
docker tag "scraper-arm64:latest" "${REGISTRY}:${VERSION}-arm64"

# Push the images
docker push "${REGISTRY}:${VERSION}-amd64"
docker push "${REGISTRY}:${VERSION}-arm64"

# Create and push the manifest
docker manifest create "${REGISTRY}:${VERSION}" "${REGISTRY}:${VERSION}-amd64" "${REGISTRY}:${VERSION}-arm64"
docker manifest push "${REGISTRY}:${VERSION}"
