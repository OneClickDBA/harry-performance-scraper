...
- Remove deprecated runtime configuration flags other than `--config.file`; scraper startup now requires a YAML config file, and process log `level` and `format` are configured under the `log` section.
...
- Set HTTP server read-header, read, and idle timeouts before handing the listener to the Prometheus web toolkit, and expose those timeout defaults through the scraper config file.
...
- Remove production use of the OCI SDK `example/helpers` package in Vault integrations so OCI Vault and HashiCorp Vault lookup failures return errors instead of terminating the scraper process.
...
- Fix custom metrics hot reload so invalid or partially written metric files are logged and ignored without crashing the scraper, preserving the last known good metric set.
- Fix custom metrics reload tracking so multiple `collector.NewScraper` instances in the same Go process each load and reload custom metrics independently.
- Add the scraper release `version` label to `oracledb_performance_scraper_build_info`, matching the standard Prometheus build info metric shape.
...
- Ignore non-string values returned from HashiCorp Vault secret payloads instead of panicking, so malformed or structured KV data cannot crash the scraper.
...
- Update the default `cache_hit_ratio` metric to query `gv$con_sysmetric`, include the `inst_id` label, and report the supported buffer and cursor cache hit ratio metrics.
- Update the Go module dependency set, including newer OCI SDK, Prometheus web toolkit, YAML, Azure SDK, and `golang.org/x/*` versions.
...
- Add per-database scrape duration metrics to complement the existing scraper-wide scrape duration metric.
...
- Add the `log.perDatabaseFiles` configuration option to write alert logs to per-database files such as `alert-db2.log`. This is recommended when scraping multiple databases from a single scraper.
- Document that TLS, basic authentication, and other Prometheus web server security settings should be configured through `web.configFile` using Prometheus Scraper Toolkit configuration.
- Start the scraper web server before initializing database connections and warming connection pools, so unavailable databases do not block the metrics endpoint from coming up.
...
- Metrics with duplicated labels no longer cause a panic. For each metric with a duplicated label, the scraper logs a warning message and skips collecting that metric. On finding warning messages, the user should adjust the scraper configuration to remove any duplicated labels.
...
- On startup, database connection pool warmup will short circuit if an error is encountered. The scraper will periodically attempt to reconnect to this database, and will not block collecting metrics from other databases.
- Update third-party dependencies.
...
- Allow loading database credentials from [HashiCorp Vault](../configuration/hashicorp-vault.md).
...
- Document required database permissions for the scraper.
...
Thank you to the following people for their suggestions and contributions:
- [@MansuyDavid](https://github.com/MansuyDavid)
- [@jdagemark](https://github.com/jdagemark)
- [@ilmarkerm](https://github.com/ilmarkerm)
...
- If the scraper fails to connect to a database due to invalid or locked credentials (ORA-01017 or ORA-28000 errors), that database configuration will be invalidated and the scraper will not attempt to re-establish the database connection. Other databases will continue to be scraped.
...
- Metrics using the `scrapeinterval` property will no longer be scraped on every request if they have a cached value. This only applies when the performance scraper is configured to scrape metrics _on request_, rather than on a global interval.
...
- Fix an issue where the scraper would unnecessarily scrape metrics with a custom scrape interval.
...
- Added WARN logging when database configurations are duplicated in the scraper configuration.
- Added INST_ID to `gv$` query metrics as a label.
- Fixed multiple concurrency bugs when the scraper is connected to multiple databases and using a custom scrape interval.
...
- Add query timeouts to initial database connections, which could cause the scraper to hang in multi-database configurations
- Fix an issue where rapidly acquiring connections could cause the scraper to crash. This was more common in multi-database configurations, due to the increased number of connection pools.
...
- Fix intermittent connection issues with ADB-S when scraper is run in a container (#169).
...
- Support for running the scraper on ARM processors (darwin and linux).
- Updated some third-party dependencies.
...
- Alert log scraper will stop if it gets three consecutive failures.
